    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3 className="font-display font-semibold text-charcoal leading-snug">{vendor.name}</h3>
        ...
        <div className="flex items-center gap-3 text-sm text-charcoal/60 mb-3">
          <span className="flex items-center gap-1">
            <FaStar className="text-gold-500" size={13} /> {vendor.rating} ({vendor.reviews})
          </span>
          <span className="flex items-center gap-1">
            <FaMapMarkerAlt size={13} /> {vendor.district}
          </span>
        </div>
        <div className="flex items-center justify-between">
          <p className="text-sm text-charcoal/70">
            From <span className="font-semibold text-maroon-600">Rs. {vendor.priceFrom.toLocaleString()}</span>
          </p>
          <Link to={`/vendors/${vendor.id}`} className="text-sm font-semibold text-gold-600 hover:text-gold-700">
            View →
          </Link>
        </div>
      </div>
