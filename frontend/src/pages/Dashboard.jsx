  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="card p-6 sm:p-8 bg-maroon-gradient text-white border-none"
      >
        <h1 className="text-2xl sm:text-3xl font-display font-bold">Welcome back</h1>
        <p className="text-ivory-100/80 mt-2 max-w-md">
      ...
      {/* Stat cards */}
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={FaWallet} label="Budget items" value={stats.budget} color="maroon" />
        <StatCard icon={FaUsers} label="Guests" value={stats.guests} color="gold" />
        <StatCard icon={FaStore} label="Vendors" value={stats.vendors} color="emerald" />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <motion.div key={a.to} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              <Link to={a.to} className="card card-hover p-5 flex items-center gap-3 text-charcoal hover:text-maroon-600">
                <div className="h-10 w-10 rounded-full bg-maroon-50 flex items-center justify-center">
                  <a.icon size={16} className="text-maroon-500" />
                </div>
