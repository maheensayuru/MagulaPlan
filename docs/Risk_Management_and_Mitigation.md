...
| **RSK-04** | Schedule & Scope | **Scope Creep on E-Commerce Payments:** Attempting real-time bank payment gateways mid-sprint threatened project delivery deadlines. | 4 (High) | 3 (Medium) | **12 (Medium)** | **Commercial Sandbox Gateway:** Decoupled live banking gateways from MVP; engineered an interactive **Commercial Payment Sandbox Gateway** on `/vendors/new` and `/vendor/dashboard` allowing full evaluation of the monetization model without merchant fees or schedule risk. | ✅ **Resolved** |
...
| **RSK-06** | Security & Privacy | **Cross-Tenant Resource Tampering (IDOR):** Risk of malicious users spoofing user IDs or vendor IDs to view or overwrite other tenants' budget, guest, or vendor data. | 3 (Medium) | 4 (High) | **12 (Medium)** | **IDOR Tenant Isolation:** Enforced server-side principal verification across budget items, guest lists, and vendor profile endpoints in Spring Security; added unit tests proving 403 Forbidden denial. | ✅ **Resolved** |
